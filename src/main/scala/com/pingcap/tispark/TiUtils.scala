...
object TiUtils {
      ...
      case Average(_) | Sum(_) | Count(_) | Min(_) | Max(_) =>
        !aggExpr.isDistinct &&
          !aggExpr.aggregateFunction.children
            .exists(expr => !isSupportedBasicExpression(expr))
      case _ => false
    }
  ...
  def isSupportedGroupingExpr(expr: NamedExpression): Boolean =
    isSupportedBasicExpression(expr)
  ...
  def toSparkDataType(tp: TiDataType): DataType =
    tp match {
      ...
      case _: DecimalType =>
        DataTypes.createDecimalType(tp.getLength, tp.getDecimal)
  ...
  def fromSparkType(tp: DataType): TiDataType =
    tp match {
      ...
      case _: sql.types.DecimalType =>
        DataTypeFactory.of(Types.TYPE_NEW_DECIMAL)
      case _: sql.types.TimestampType =>
        DataTypeFactory.of(Types.TYPE_TIMESTAMP)
