...
private[hive] object TiSparkSQLCLIDriver extends Logging {
    ...
    HiveInterruptUtils.add(new HiveInterruptCallback {
      override def interrupt() {
        ...
        if (TiSparkSQLEnv.sparkContext != null) {
          TiSparkSQLEnv.sparkContext.cancelAllJobs()
        } else {
          if (transport != null) {
  ...
  def main(args: Array[String]) {
    ...
    val cli = new TiSparkSQLCLIDriver
    ...
    while (it.hasNext) {
      val kv = it.next()
      TiSparkSQLEnv.sqlContext.setConf(kv.getKey, kv.getValue)
    }
...
private[hive] class TiSparkSQLCLIDriver extends CliDriver with Logging {
  ...
  private val isRemoteMode = {
    TiSparkSQLCLIDriver.isRemoteMode(sessionState)
  }

  private val conf: Configuration =
    if (sessionState != null) sessionState.getConf else new Configuration()

  // Force initializing TiSparkSQLEnv. This is put here but not object TiSparkSQLCliDriver
