...
class TiStrategy(context: SQLContext) extends Strategy with Logging {
  ...
  def aggregationToSelectRequest(
    groupByList: Seq[NamedExpression],
    aggregates: Seq[AggregateExpression],
    source: TiDBRelation,
    selectRequest: TiSelectRequest = new TiSelectRequest
  ): TiSelectRequest = {
    aggregates.foreach {
      ...
      case AggregateExpression(f @ Count(args), _, _, _) =>
        ...
        selectRequest.addAggregate(
          new TiCount(tiArgs: _*),
  ...
  def filterToSelectRequest(
    filters: Seq[Expression],
    source: TiDBRelation,
    selectRequest: TiSelectRequest = new TiSelectRequest
  ): TiSelectRequest = {
    val tiFilters: Seq[TiExpr] = filters.collect {
    ...
    val scanPlan = scanBuilder.buildScan(
      JavaConversions.seqAsJavaList(tiFilters),
      pkIndex,
      source.table
    )
  ...
  def pruneFilterProject(
    ...
    if (AttributeSet(projectList.map(_.toAttribute)) == projectSet &&
      ...
      projectSeq.foreach(
        attr => selectRequest.addField(TiColumnRef.create(attr.name))
      )
  ...
  private def doPlan(source: TiDBRelation, plan: LogicalPlan): Seq[SparkPlan] = {
    ...
    def toAlias(expr: Expression) =
      aliasMap.getOrElseUpdate(expr, Alias(expr, expr.toString)())
      ...
      case TiAggregation(
        ...
        val residualAggregateExpressions = aggregateExpressions.map { aggExpr =>
          aggExpr.aggregateFunction match {
            ...
            case Max(_) =>
              newAggregate(Max(toAlias(aggExpr).toAttribute), aggExpr)
            case Min(_) =>
              newAggregate(Min(toAlias(aggExpr).toAttribute), aggExpr)
            case Count(_) =>
              newAggregate(Sum(toAlias(aggExpr).toAttribute), aggExpr)
            case Sum(_) =>
              newAggregate(Sum(toAlias(aggExpr).toAttribute), aggExpr)
            case First(_, ignoreNullsExpr) =>
              newAggregate(
                First(toAlias(aggExpr).toAttribute, ignoreNullsExpr),
                aggExpr
              )
            case _ => aggExpr
          }
          ...
          aggExpr match {
            ...
            case aggExpr @ AggregateExpression(Average(ref), _, _, _) =>
              ...
              val sumFinal =
                newAggregateWithId(Sum(toAlias(sumToPush).toAttribute), aggExpr)
              val countFinal =
                newAggregateWithId(
                  Sum(toAlias(countToPush).toAttribute),
                  aggExpr
                )
        ...
        selReq = aggregationToSelectRequest(
          groupingExpressions,
          pushdownAggregates,
          source,
          selReq
        ...
        val output =
          (groupingExpressions ++ pushdownAggregates.map(x => toAlias(x)))
            .map(_.toAttribute)
