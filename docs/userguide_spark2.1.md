...
Please refer to the Spark official configuration website at (https://spark.apache.org/docs/latest/spark-standalone.html). The following is an example based on spark-env.sh configuration:
...
To use Thrift Server, you can start it a similar way as default Spark Thrift Server:
```
...
// Another option is manually loaded by yourself, yet this is not the recommended way.
