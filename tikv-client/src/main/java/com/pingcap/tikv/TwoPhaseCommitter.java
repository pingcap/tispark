...
public class TwoPhaseCommitter {
  ...
  public TwoPhaseCommitter(TiConfiguration conf, long startTime) {
    this.kvClient = ClientSession.getInstance(conf).getTikvSession().createTxnClient();
  ...
  private void doPrewritePrimaryKeyWithRetry(BackOffer backOffer, ByteString key, ByteString value)
    ...
    ClientRPCResult prewriteResult =
        this.kvClient.prewrite(
            backOffer, mutationList, key, lockTTL, this.startTs, tiRegion, tiStore);
  ...
  private void doCommitPrimaryKeyWithRetry(BackOffer backOffer, ByteString key, long commitTs)
    ...
    ClientRPCResult commitResult =
        this.kvClient.commit(
            backOffer, keys.toArray(new ByteString[0]), this.startTs, commitTs, tiRegion, tiStore);
  ...
  private void doPrewriteSecondaryKeysInBatchesWithRetry(
    ...
    GroupKeyResult groupResult =
        this.groupKeysByRegion(keys.toArray(new ByteString[0]), size, backOffer);
    ...
    for (Map.Entry<Pair<TiRegion, TiStore>, List<ByteString>> entry : groupKeyMap.entrySet()) {
      ...
      TiStore tiStore = entry.getKey().second;
      this.appendBatchBySize(batchKeyList, tiRegion, tiStore, entry.getValue(), true, mutations);
    }
  ...
  private void appendBatchBySize(
    ...
    for (start = 0; start < len; start = end) {
      ...
      BatchKeys batchKeys = new BatchKeys(tiRegion, tiStore, keys.subList(start, end), sizeInBytes);
  ...
  private void doCommitSecondaryKeySingleBatchWithRetry(
    ...
    ClientRPCResult commitResult =
        this.kvClient.commit(
            backOffer,
            keysCommit.toArray(new ByteString[0]),
            this.startTs,
            commitTs,
            batchKeys.getRegion(),
            batchKeys.getStore());
