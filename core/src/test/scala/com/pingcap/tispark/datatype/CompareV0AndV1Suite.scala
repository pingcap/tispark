...
class CompareV0AndV1Suite extends BaseDataSourceTest("test_data_type_compare_v0_v1", "test") {
  ...
  test("Test compare BIT(4) Type") {
    ...
    val answerV1 = toByteArray(0) :: toByteArray(5) :: toByteArray(10) :: toByteArray(15) :: Nil
  ...
  test("Test compare BIT(8) Type") {
    ...
    val answerV1 = toByteArray(0) :: toByteArray(85) :: toByteArray(170) :: toByteArray(255) :: Nil
    val skipSparkJDBC = true // cause spark jdbc return BooleanType
  ...
  test("Test compare BIT(10) Type") {
    ...
    val answerV1 = toByteArray(0, 0) :: toByteArray(1, 85) :: toByteArray(2, 170) :: toByteArray(
      3,
      255
    ) :: Nil
    val skipSparkJDBC = true // cause spark jdbc return BooleanType
  ...
  test("Test compare BIT(64) Type") {
    ...
    val b1 = toByteArray(0, 0, 0, 0, 0, 0, 0, 0)
    val b2 = toByteArray(85, 85, 85, 85, 85, 85, 85, 85)
    val b3 = toByteArray(255, 255, 255, 255, 255, 255, 255, 255)
  ...
  private def doTestSelect(createTableQuery: String,
    ...
    if (!skipJDBC) {
      val sortedResJDBC = resJDBC.sortBy(l => l.head.toString.toInt)
      for (i <- sortedResJDBC.indices) {
        assert(isEqual(answerV1(i), sortedResJDBC(i)(1)))
      }
    }

    // select using spark jdbc and assert
    val dfSparkJDBC = queryViaSparkJDBC(s"(select * from `$database`.`$table` $where) as tmp")
    val sortedResSparkJDBC = dfSparkJDBC.collect().sortBy(l => l(0).toString.toInt)
    if (!skipSparkJDBC) {
      for (i <- sortedResSparkJDBC.indices) {
        assert(isEqual(answerV1(i), sortedResSparkJDBC(i).get(1)))
    ...
    val sortedResSparkTiDBV0 = dfSparkTiDBV0.collect().sortBy(l => l(0).toString.toInt)
    for (i <- sortedResSparkTiDBV0.indices) {
      assert(isEqual(answerV0(i), sortedResSparkTiDBV0(i).get(1)))
      assert(!isEqual(answerV1(i), sortedResSparkTiDBV0(i).get(1)))
    }
    ...
    val sortedResSparkTiDBV1 = dfSparkTiDBV1.collect().sortBy(l => l(0).toString.toInt)
    for (i <- sortedResSparkTiDBV1.indices) {
      assert(isEqual(answerV1(i), sortedResSparkTiDBV1(i).get(1)))
      assert(!isEqual(answerV0(i), sortedResSparkTiDBV1(i).get(1)))
    }
  }
