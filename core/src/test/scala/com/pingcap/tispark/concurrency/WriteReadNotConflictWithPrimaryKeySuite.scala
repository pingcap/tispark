...
class WriteReadNotConflictWithPrimaryKeySuite extends ConcurrentcyTest {
  test("read not conflict with primary key using jdbc") {
    if (!supportBatchWrite) {
      cancel
    }

    dropTable()
    jdbcUpdate(s"create table $dbtable(i int, s varchar(128), PRIMARY KEY(i))")
  ...
  test("read not conflict with primary key using tispark") {
    if (!supportBatchWrite) {
      cancel
    }
