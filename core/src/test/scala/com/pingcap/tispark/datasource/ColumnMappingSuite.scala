...
class ColumnMappingSuite extends BaseDataSourceTest("test_datasource_insert_with_different_column_order") {
  ...
  private def generateData(start : Int, length : Int, posMap : List[Int], skipFirstCol : Boolean = false): (List[Row], List[Row]) = {
  ...
  test("Test different column order with full schema") {
    dropTable()
    jdbcUpdate(s"create table $dbtable(i int primary key auto_increment, s varchar(128), c varchar(128))")
