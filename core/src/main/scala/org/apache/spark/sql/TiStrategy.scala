...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, AttributeSet, Cast, Divide, ExprId, Expression, IntegerLiteral, NamedExpression, SortOrder}
...
//      source.table.setPartitionExpr(generatePartitionExpr(source.table.getPartitionInfo))
  ...
  private def groupAggregateProjection(
    ...
    val deterministicAggAliases: Map[Expression, Alias] = aggregateExpressions.collect {
    ...
    val residualAggregateExpressions: Seq[AggregateExpression] = aggregateExpressions.map {
      aggExpr =>
        ...
        val partialResultRef: Attribute = aliasPushedPartialResult(aggExpr).toAttribute
    ...
    val groupAttributes: Seq[Attribute] = groupingExpressions.map(_.toAttribute)

    // output of Coprocessor plan should contain all references within
    // aggregates and group by expressions
    val output: Seq[Attribute] = aggregateAttributes ++ groupAttributes

    val groupExpressionMap: Map[ExprId, Attribute] =
    ...
    val rewrittenResultExpressions: Seq[NamedExpression] = resultExpressions.map {
      _.transform {
