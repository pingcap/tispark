      ...
      .mapPartitionsWithIndexInternal { (index, iter) =>
        ...
        iter.flatMap { row =>
            ...
            for (task <- tasks) {
              ...
              while (queue.nonEmpty) {
                ...
                if (isTaskRangeSizeInvalid(front)) {
                  ...
                  front.getRanges.grouped((task.getRanges.size() + 1) / 2).foreach(range => {
          ...
          val schemaInferrer: SchemaInfer = if (satisfyDowngradeThreshold) {
            ...
            val taskRanges = indexTasks.head.getRanges
            logger.warn(
              s"Index task range size = ${indexTasks.size()} exceed downgrade threshold:$downgradeThreshold, " +
                s"index scan handle size = ${handles.length}, " +
                s"downgrade to table scan with ${indexTasks.size()} region tasks, " +
