...
object TiBatchWriteBigDataTest {
  def main(args: Array[String]): Unit = {
    val outputDatabase = "test"
    val outputTable = "RANDOM_DATA"
    val enableRegionPreSplit = false
    val regionSplitNumber = None
    val tikvInstanceNumber = 1
    ...
    val rdd: RDD[Row] = spark.sparkContext.makeRDD(1 to 1033907, 12).map { i =>
      Row(i)
    }
