...
class TiDBOptions(@transient val parameters: CaseInsensitiveMap[String]) extends Serializable {
  ...
  if (SSLParameters.equals("true")) {
    ...
    if (serverCertStore.equals("")) {
      SSLParameters = "true&requireSSL=true&trustServerCertificate=true"
    } else {
      if (!serverCertStore.startsWith(FILE_PREFIX))
        serverCertStore = FILE_PREFIX + serverCertStore
      SSLParameters =
        "true&requireSSL=true&trustServerCertificate=false&trustStore=" +
  ...
  // ------------------------------------------------------------
  val url: String =
    s"jdbc:mariadb://address=(protocol=tcp)(host=$address)(port=$port)/?user=$user&password=$password&useSSL=$SSLParameters&rewriteBatchedStatements=true&enabledSslProtocolSuites=TLSv1.2,TLSv1.3"
      .replaceAll("%", "%25")
